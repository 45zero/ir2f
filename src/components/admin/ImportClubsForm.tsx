...
import { importClubsExcel } from "@/lib/actions/clubs"
import { colors, fontBody } from "@/lib/theme"

export function ImportClubsForm() {
  const [state, formAction, isPending] = useActionState(importClubsExcel, undefined)
  ...
  return (
    ...
    >
      ...
      >
        {isPending ? "Import..." : "Importer l'annuaire"}
      </button>
      ...
      {state && !state.error && state.imported !== null && (
        <span style={{ color: colors.navy, fontSize: 12.5, fontWeight: 700 }}>
          {state.imported} club{state.imported > 1 ? "s" : ""} importé{state.imported > 1 ? "s" : ""}.
        </span>
      )}
    </form>
  )
